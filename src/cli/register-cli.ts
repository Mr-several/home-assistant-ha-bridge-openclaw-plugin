...
function formatStatusForText(status: HaBridgeStatus): string {
  const lines = [
...
export function registerHaBridgeCli(params: CliParams) {
  ...
  root
    ...
    .action((opts: { json?: boolean }) => {
      ...
      const status: HaBridgeStatus = service?.getStatus() ?? {
        running: false,
        connected: false,
        configured: false,
        missingRequired: [],
  ...
  root
    ...
    .option("--json", "Print JSON output", false)
    .action((opts: { mode: string; message: string; json?: boolean }) => {
      const service = params.getService();
      if (!service) {
        throw new Error("ha-bridge service is not running");
      }
