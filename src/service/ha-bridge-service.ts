...
import { getMissingRequired, parseHaBridgeConfig, type HaBridgeConfig } from "../config.js";
import {
  CommandDispatcher,
  type CommandRunner,
  type DispatchResult,
} from "../dispatch/command-dispatcher.js";
import { HaWsClient, type HaEvent, type HaWsClientLogger } from "../ha/ws-client.js";
...
import { RuntimeState, type HaBridgeStatus } from "../state/runtime-state.js";
...
type AgentSelectionSource = "configured" | "detected" | "fallback";
...
type Route =
  | { kind: "explicit"; channel: string; target: string }
  | { kind: "last" };

type DirectRoute = { channel: string; target: string };
type DirectRouteCandidate = DirectRoute & { agentId: string; updatedAt: number };

export class HaBridgeService {
  private config: HaBridgeConfig | null = null;
  private effectiveAgentId = "main";
  private state = new RuntimeState();
  private deduper: EventDeduper | null = null;
  private limiter: SlidingWindowRateLimiter | null = null;
  private queue: BoundedAsyncQueue<HaEvent> | null = null;
  private dispatcher: CommandDispatcher | null = null;
  private wsClient: HaWsClient | null = null;

  constructor(private readonly deps: HaBridgeServiceDeps) {}
  ...
  async start(): Promise<void> {
    ...
    if (missingRequired.length > 0) {
      ...
      this.deps.logger.warn?.(
        `ha-bridge not configured; missing required config: ${missingSummary}`,
      );
      return;
    }

    let config: HaBridgeConfig;
    try {
    ...
    if (effectiveAgent.source === "detected") {
      this.deps.logger.info?.(
        `ha-bridge using detected default agentId: ${this.effectiveAgentId}`,
      );
    ...
    this.queue = new BoundedAsyncQueue<HaEvent>(config.queueMax, (error) => {
  ...
  dryRun(params: { mode: "direct" | "agent"; message: string }): DryRunResult {
    ...
    if (params.mode === "direct") {
      ...
      if (route.kind === "last") {
        ...
        if (!lastRoute) {
          throw new Error(
            "ha-bridge direct route fallback has no previous channel/target; configure route or send one chat first",
          );
        }
        return {
          mode: "direct",
  ...
  async ingestEventForTest(event: HaEvent): Promise<void> {
    await this.onInboundEvent(event);
    await new Promise((resolve) => setTimeout(resolve, 0));
  }

  private async onInboundEvent(event: HaEvent): Promise<void> {
    const config = this.requireConfig();
  ...
  private async consumeEvent(event: HaEvent): Promise<void> {
    const config = this.requireConfig();
    const dispatcher = this.requireDispatcher();
    ...
    if (event.event_type === config.directEventType) {
      ...
      if (result.ok) {
        this.state.setLastDispatch(
          "direct_message",
          route.kind === "explicit" ? "ok" : "ok:last_route",
        );
        return;
      }
      ...
      this.deps.logger.warn?.(
        `ha-bridge direct dispatch failed${
          route.kind === "last" ? " (last route fallback)" : ""
  ...
  private resolveRoute(mode: "direct" | "agent"): Route {
    const config = this.requireConfig();
  ...
  private async dispatchDirectViaLastRoute(message: string): Promise<DispatchResult> {
    const dispatcher = this.requireDispatcher();
    const lastRoute = this.resolveLastDirectRoute();
    if (!lastRoute) {
      return {
        ...
        stderr: "",
        error:
          "missing last route: no previous channel/target found for direct fallback; configure route or send one chat first",
      };
    }
  ...
  private async dispatchAgentWithFallback(params: {
    agentId?: string;
    sessionId: string;
    prompt: string;
    channel?: string;
    target?: string;
    thinking: HaBridgeConfig["thinking"];
    ...
    this.deps.logger.warn?.(
      `ha-bridge agentId "${params.agentId}" is invalid; retrying with default agent`,
    );
  ...
  private isUnknownAgentError(result: DispatchResult): boolean {
    const haystack = `${result.stderr}\n${result.stdout}\n${result.error ?? ""}`;
    return haystack.includes("Unknown agent id");
  }

  private resolveLastDirectRoute(): DirectRoute | null {
    ...
    if (fallback) {
      this.deps.logger.warn?.(
        `ha-bridge direct last-route fallback using agent "${fallback.agentId}" from recent session`,
      );
      return {
    ...
    this.deps.logger.debug?.(
      `ha-bridge direct last-route lookup failed: sessionKey=${sessionKey}, storePath=${storePath}`,
    );
    return null;
  }

  private resolveEffectiveAgent(config: HaBridgeConfig): {
    agentId: string;
  ...
  private buildDryRunEvent(mode: "direct" | "agent", message: string): HaEvent {
    const config = this.requireConfig();
    return {
  ...
  private resolveDispatchMode(eventType: string): "direct_message" | "agent_rewrite" {
    const config = this.requireConfig();
    return eventType === config.directEventType ? "direct_message" : "agent_rewrite";
  }

  private requireConfig(): HaBridgeConfig {
    if (!this.config) {
      throw new Error("ha-bridge service is not running");
    }
    return this.config;
  ...
  private requireDeduper(): EventDeduper {
    if (!this.deduper) {
  ...
  private requireLimiter(): SlidingWindowRateLimiter {
    if (!this.limiter) {
  ...
  private requireQueue(): BoundedAsyncQueue<HaEvent> {
    if (!this.queue) {
  ...
  private requireDispatcher(): CommandDispatcher {
    if (!this.dispatcher) {
...
function detectDefaultAgentId(gatewayConfig: unknown): string | undefined {
...
function buildMainSessionKey(gatewayConfig: unknown, agentId: string): string {
  const root = asRecord(gatewayConfig);
...
function resolveSessionStorePath(gatewayConfig: unknown, agentId: string): string {
  const root = asRecord(gatewayConfig);
  const session = asRecord(root.session);
...
function isDeliverableRoute(route: DirectRoute): boolean {
...
function pickRoute(channel: unknown, target: unknown): DirectRoute | null {
  const route = {
...
function loadLastRouteFromStore(storePath: string, sessionKey: string): DirectRoute | null {
...
function pickRouteWithTimestamp(
  agentId: string,
  entry: Record<string, unknown>,
): DirectRouteCandidate | null {
...
function firstRouteFromEntry(entry: Record<string, unknown>): DirectRoute | null {
  const deliveryContext = asRecord(entry.deliveryContext);
  const candidates: Array<DirectRoute | null> = [
...
function findLatestDirectRouteFromAllAgents(stateDir: string): DirectRouteCandidate | null {
  ...
  let latest: DirectRouteCandidate | null = null;
